...
V8.7 = V8.6 + lowercase headers (V7-compatible) + multi-structure extract
     + typeId debug + raw response dump.

Fixes:
- Headers lowercase (V7 compatible)
- typeId tested via env
- Full raw response visible in /api/debug
"""
...
API_TYPE_ID = int(os.getenv("API_TYPE_ID", "1"))  # V8.7 default 1 (V7 used 30)
...
class Database:
    ...
    def save_result(self, period, number, result):
        with self.lock:
            self.conn.execute("""INSERT OR IGNORE INTO history
                (period, number, result, timestamp) VALUES (?, ?, ?, ?)""",
...
# ============================================================
# API CLIENT (V8.7 — lowercase headers)
# ============================================================

def fetch_api_raw():
    """V8.7 — lowercase headers to match V7 (working) code"""
    headers = {
    ...
    if API_AUTH:
        headers["authorization"] = f"Bearer {API_AUTH}"  # lowercase
    if API_ORIGIN:
        headers["origin"] = API_ORIGIN  # lowercase
    if API_REFERER:
        headers["referer"] = API_REFERER  # lowercase
    ...
    while True:
        try:
            ...
            for u in r.json().get("result", []):
                ...
                if txt == "/status":
                    rep = agent.db.report()
                    telegram(f"🚀 V8.7\nHistory: {len(agent.db.history())}\nWR: {rep['wr']*100:.1f}%" if rep['wr'] else "🚀 V8.7\nNo data yet")
                elif txt == "/help":
...
<!doctype html>
<html><head><meta charset="utf-8"><title>V8.7</title>
<style>body{background:#0f172a;color:#f8fafc;font-family:system-ui;padding:20px;margin:0}
h1{color:#22d3ee}.card{background:#1e293b;padding:14px;border-radius:10px;margin-bottom:12px}
.big{font-size:24px;font-weight:800}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px}</style>
</head><body>
<h1>🚀 HYBRID V8.7</h1>
<div class="grid">
<div class="card"><div>Evaluated</div><div class="big">{{ report.total }}</div></div>
<div class="card"><div>WR</div><div class="big">{{ "%.1f"|format(report.wr*100) if report.wr else "N/A" }}%</div></div>
</div>
<div class="card">
{% if p %}<p>Latest: {{ p.prediction }} — {{ "%.1f"|format(p.confidence*100) }}% [{{ p.state }}]</p>
{% else %}<p>No signal yet.</p>{% endif %}
</div>
</body></html>""", p=p, report=report)
...
def api_debug():
    ...
    try:
        ...
        result["headers_sent"] = {k: (v[:50] + "..." if isinstance(v, str) and len(v) > 50 else v) for k, v in headers.items()}
        ...
        if parsed_json and isinstance(parsed_json, dict):
            ...
            if "data" in parsed_json:
                ...
                if isinstance(d, dict):
                    ...
                    extraction_attempts["data_is_list_len"] = len(d)
...
if __name__ == "__main__":
    ...
    try:
        ...
        log(f"[STARTUP] payload sent: {payload}")
        log(f"[STARTUP] header keys: {list(headers.keys())}")
